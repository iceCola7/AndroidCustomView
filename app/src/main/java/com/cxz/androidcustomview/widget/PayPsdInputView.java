...
public class PayPsdInputView extends AppCompatEditText {
    ...
    private final static int psdType_rectBox = 0;
    ...
    private void init(AttributeSet attrs) {
        ...
        divideLineColor = typedArray.getColor(R.styleable.PayPsdInputView_dividerLineColor, divideLineColor);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (psdType) {
            case psdType_rectBox:
                drawRectBoxBorder(canvas);
     ...
     * @param canvas
     */
    private void drawRectBoxBorder(Canvas canvas) {
        canvas.drawRoundRect(rectF, rectAngle, rectAngle, borderPaint);

        for (int i = 0; i < maxCount - 1; i++) {
            canvas.drawLine((i + 1) * divideLineWStartX, 0,
                    (i + 1) * divideLineWStartX, height, divideLinePaint);
        }
    }
    ...
    private void drawItemFocused(Canvas canvas, int position) {
        ...
        focusedRecF.set(position * divideLineWStartX, 0,
                (position + 1) * divideLineWStartX, height);
    ...
    private void drawBottomBorder(Canvas canvas) {
        for (int i = 0; i < maxCount; i++) {
            ...
            canvas.drawLine(cX - bottomLineLength / 2, height,
                    cX + bottomLineLength / 2, height, bottomLinePaint);
        }
    }
        ...
        if (textLength == maxCount) {
            if (mListener != null) {
                if (TextUtils.isEmpty(mComparePassword)) {
                    mListener.onFinished(getPasswordString());
                } else {
    ...
    public interface onPasswordListener {
        ...
        void onFinished(String inputPsd);
    }
}
